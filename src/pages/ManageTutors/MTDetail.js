...
const MTDetail = () => {
  ...
  const TutorDB = TutorsData.find((TDB) => TDB.id === newId);
    ...
    <>
      ...
      <div className="flex flex-col w-full h-full px-6 md:pl-[300px] md:pt-10 space-y-12">
        ...
        <div className="flex flex-row items-center space-x-4">
          <img
            src={TutorDB.imageUrl}
            alt={TutorDB.name}
            className="w-32 h-32 p-1 border-gray-500 bg-gray-100 rounded-full border-[1px]"
          />
          <div className="flex flex-col -space-y-1">
            <h1 className="text-xl">{TutorDB.name}</h1>
            <p className="">{TutorDB.role}</p>
            <p className="">{TutorDB.school}</p>
          </div>
        </div>
